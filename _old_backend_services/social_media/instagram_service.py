    ...
    async def get_comprehensive_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if recent_media and "data" in recent_media:
                total_likes = sum(post.get("like_count", 0) for post in recent_media["data"])
                total_comments = sum(post.get("comments_count", 0) for post in recent_media["data"])
                metrics["recent_engagement"] = {
                    "total_likes": total_likes,
                    "total_comments": total_comments,
                    "avg_likes_per_post": total_likes / len(recent_media["data"]) if recent_media["data"] else 0,
                    "avg_comments_per_post": total_comments / len(recent_media["data"]) if recent_media["data"] else 0
                }
