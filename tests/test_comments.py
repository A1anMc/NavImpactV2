...
def test_create_comment(
    authorized_client: TestClient, db: Session, test_user, test_task
):
    """Test creating a comment."""
    # Create comment data
    comment_data = {"content": "Test comment", "task_id": test_task.id, "mentions": []}

    # Create comment
    response = authorized_client.post("/api/v1/comments/", json=comment_data)
...
def test_create_threaded_comment(
    authorized_client: TestClient, db: Session, test_user, test_task
):
    """Test creating a threaded comment."""
    # Create parent comment
    parent_comment = TaskComment(
        content="Parent comment", task_id=test_task.id, user_id=test_user.id
    )
...
def test_get_task_comments(
    ...
    comments = [
        TaskComment(
            content=f"Test comment {i}", task_id=test_task.id, user_id=test_user.id
        )
        for i in range(3)
    ]
    db.add_all(comments)
    ...
    # Get comments
    response = authorized_client.get(f"/api/v1/comments/task/{test_task.id}/")
    assert response.status_code == 200
...
def test_update_comment(
    ...
    # Create test comment
    comment = TaskComment(
        content="Original content", task_id=test_task.id, user_id=test_user.id
    )
    db.add(comment)
    db.commit()
    db.refresh(comment)
    ...
    # Update comment data
    update_data = {"content": "Updated content", "mentions": []}
...
def test_delete_comment(
    ...
    # Create test comment
    comment = TaskComment(
        content="Test comment", task_id=test_task.id, user_id=test_user.id
    )
    db.add(comment)
    db.commit()
    db.refresh(comment)
...
def test_add_reaction(authorized_client: TestClient, db: Session, test_user, test_task):
    ...
    reaction = (
        db.query(Reaction)
        .filter(Reaction.comment_id == comment.id, Reaction.user_id == test_user.id)
        .first()
    )
...
def test_remove_reaction(
    ...
    # Add reaction
    reaction = Reaction(emoji="👍", comment_id=comment.id, user_id=test_user.id)
    db.add(reaction)
    db.commit()

    # Remove reaction
    response = authorized_client.delete(f"/api/v1/comments/{comment.id}/reactions/👍/")
    assert response.status_code == 200
