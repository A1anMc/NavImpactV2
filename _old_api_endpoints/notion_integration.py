...
from app.models.notion_integration import NotionWorkspace, NotionSyncMapping, NotionSyncLog
from app.models.sge_media import SgeMediaProject
from app.schemas.notion_integration import (
    NotionWorkspaceCreate, NotionWorkspaceUpdate, NotionWorkspaceResponse,
    NotionSyncMappingCreate, NotionSyncMappingUpdate, NotionSyncMappingResponse,
    NotionSyncLogResponse, NotionConnectionRequest, NotionSyncRequest,
    NotionTemplateRequest, NotionSyncStatus, NotionSyncDashboard,
    NotionWorkspaceFilters, NotionSyncMappingFilters, NotionSyncLogFilters,
    PaginatedNotionWorkspaces, PaginatedNotionSyncMappings, PaginatedNotionSyncLogs,
    SyncDirection, SyncStatus, OperationType, EntityType, LogStatus
)
from app.services.notion_client import NotionAPIClient, NotionTemplateManager, NotionSyncManager, NotionAPIError
...
async def connect_notion_workspace(
    connection_request: NotionConnectionRequest,
...
async def get_notion_workspaces(
    ...
    if workspace_name:
        query = query.filter(NotionWorkspace.workspace_name.ilike(f"%{workspace_name}%"))
...
async def get_notion_workspace(
    ...
    workspace = db.query(NotionWorkspace).filter(NotionWorkspace.id == workspace_id).first()
    if not workspace:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Notion workspace not found"
        )
    return workspace
...
async def delete_notion_workspace(
    workspace_id: int,
    db: Session = Depends(get_db)
):
...
async def sync_media_project_to_notion(
    ...
    try:
        ...
        project = db.query(SgeMediaProject).filter(SgeMediaProject.id == project_id).first()
        if not project:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Media project not found"
            )
        
        # Get active workspace
        workspace = db.query(NotionWorkspace).filter(NotionWorkspace.is_active == True).first()
        if not workspace:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        mapping = db.query(NotionSyncMapping).filter(
            NotionSyncMapping.media_project_id == project_id
        ).first()
        ...
        if not mapping:
            ...
            notion_page = template_manager.create_media_project_template(parent_page_id, project_data)
            
            # Create mapping
            mapping = NotionSyncMapping(
                media_project_id=project_id,
                notion_page_id=notion_page["id"],
                sync_direction=sync_request.sync_direction or SyncDirection.BIDIRECTIONAL,
            ...
            notion_client = NotionAPIClient(workspace.access_token)
            sync_manager = NotionSyncManager(notion_client, NotionTemplateManager(notion_client))
...
async def get_notion_sync_status(db: Session = Depends(get_db)):
    ...
    pending_mappings = db.query(NotionSyncMapping).filter(
        NotionSyncMapping.sync_status == SyncStatus.PENDING
    ).count()
    failed_mappings = db.query(NotionSyncMapping).filter(
        NotionSyncMapping.sync_status == SyncStatus.FAILED
    ).count()
    
    # Get recent errors
    recent_errors = db.query(NotionSyncLog).filter(
        and_(
            NotionSyncLog.workspace_id == workspace.id,
            NotionSyncLog.status == LogStatus.FAILED
        )
    ).order_by(NotionSyncLog.created_at.desc()).limit(5).all()
    
    recent_error_messages = [log.error_message for log in recent_errors if log.error_message]
    
    # Get last sync time
    last_sync = db.query(NotionSyncMapping).filter(
        NotionSyncMapping.last_sync_at.isnot(None)
    ).order_by(NotionSyncMapping.last_sync_at.desc()).first()
    ...
    return NotionSyncStatus(
        ...
        sync_status=SyncStatus.COMPLETED if pending_mappings == 0 else SyncStatus.SYNCING,
...
async def get_notion_sync_logs(
    ...
    logs = query.order_by(NotionSyncLog.created_at.desc()).offset(skip).limit(limit).all()
...
async def create_notion_template(
    template_request: NotionTemplateRequest,
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        if template_request.template_name == "media_project":
            result = template_manager.create_media_project_template(
                template_request.parent_page_id,
                template_request.project_data or {}
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_notion_dashboard(db: Session = Depends(get_db)):
    ...
    # Get recent logs
    recent_logs = db.query(NotionSyncLog).filter(
        NotionSyncLog.workspace_id == workspace.id
    ).order_by(NotionSyncLog.created_at.desc()).limit(10).all()
    
    # Calculate sync analytics
    total_syncs = db.query(NotionSyncLog).filter(
        NotionSyncLog.workspace_id == workspace.id
    ).count()
    
    successful_syncs = db.query(NotionSyncLog).filter(
        and_(
            NotionSyncLog.workspace_id == workspace.id,
    ...
    failed_syncs = db.query(NotionSyncLog).filter(
        and_(
            NotionSyncLog.workspace_id == workspace.id,
            NotionSyncLog.status == LogStatus.FAILED
        )
