...
class GrantRepository(BaseRepository[Grant]):
    ...
    def get_grants_by_location(self, location: str) -> List[Grant]:
        ...
        return (
            self.db.query(Grant)
            .filter(Grant.location_eligibility.contains(location))
            .all()
        )

    def get_grants_by_amount_range(
        self, min_amount: int, max_amount: int
    ) -> List[Grant]:
        """Get grants within amount range"""
        return (
            self.db.query(Grant)
            .filter(and_(Grant.amount >= min_amount, Grant.amount <= max_amount))
            .all()
        )
    ...
    def get_grants_by_deadline(self, days_ahead: int = 30) -> List[Grant]:
        ...
        return (
            self.db.query(Grant)
            .filter(
                and_(Grant.deadline >= datetime.utcnow(), Grant.deadline <= cutoff_date)
            )
            .order_by(asc(Grant.deadline))
            .all()
        )
    ...
    def get_grants_by_multiple_criteria(self, criteria: Dict[str, Any]) -> List[Grant]:
        ...
        if criteria.get("location"):
            query = query.filter(
                Grant.location_eligibility.contains(criteria["location"])
            )
    ...
    def get_recent_grants(self, days: int = 7) -> List[Grant]:
        ...
        return (
            self.db.query(Grant)
            .filter(Grant.created_at >= cutoff_date)
            .order_by(desc(Grant.created_at))
            .all()
        )
    ...
    def get_grant_statistics(self) -> Dict[str, Any]:
        ...
        for source in sources:
            source_counts[source[0]] = (
                self.db.query(Grant).filter(Grant.source == source[0]).count()
            )
