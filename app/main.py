...
if settings.ENV == "production" and settings.RATE_LIMIT_ENABLED:
    try:
        ...
        logger.info(
            f"Rate limiting enabled: {settings.RATE_LIMIT_REQUESTS_PER_MINUTE}/min, {settings.RATE_LIMIT_REQUESTS_PER_HOUR}/hour"
        )
    except ImportError:
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        servers=(
            [{"url": "/", "description": "NavImpact API Server"}]
    ...
    async def error_logging_middleware(request: Request, call_next):
        ...
        try:
            ...
            duration = time.time() - start_time
            logger.error(f"{method} {path} - ERROR - {duration:.3f}s - {str(e)}")
