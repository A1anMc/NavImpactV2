...
class DatabasePoolMonitor:
    ...
    def get_pool_metrics(self) -> Optional[PoolMetrics]:
        ...
        try:
            ...
            utilization_percent = (
                (checked_out / max_pool_size) * 100 if max_pool_size > 0 else 0
            )
            overflow_utilization_percent = (
                (overflow / max_overflow) * 100 if max_overflow > 0 else 0
            )
    ...
    def _check_alerts(self, metrics: PoolMetrics):
        ...
        if metrics.utilization_percent > 90:
            logger.error(
                "🔴 CRITICAL: Pool near exhaustion - consider increasing pool size"
            )
    ...
    def get_pool_summary(self) -> Dict:
        ...
        if recent_metrics:
            avg_utilization = sum(m.utilization_percent for m in recent_metrics) / len(
                recent_metrics
            )
            max_utilization = max(m.utilization_percent for m in recent_metrics)
            avg_checked_out = sum(m.checked_out for m in recent_metrics) / len(
                recent_metrics
            )
        else:
    ...
    def _get_recommendations(self, metrics: PoolMetrics) -> List[str]:
        ...
        if metrics.utilization_percent > 90:
            recommendations.append(
                "🔴 CRITICAL: Increase DATABASE_POOL_SIZE immediately"
            )
            recommendations.append(
                "Consider scaling down concurrent scraper operations"
            )
            ...
            recommendations.append(
                "🟡 Using overflow connections - consider increasing pool size"
            )
...
class ConnectionMonitor:
    ...
    def __enter__(self):
        ...
        if metrics:
            logger.debug(
                f"Connection acquired for {self.operation_name} "
                f"(pool utilization: {metrics.utilization_percent:.1f}%)"
            )
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            logger.debug(
                f"Connection released after {duration:.2f}s for {self.operation_name}"
            )
        ...
        if metrics and metrics.is_critical():
            logger.warning(
                f"High pool utilization after {self.operation_name}: "
