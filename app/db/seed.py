...
def create_test_tasks(
    db: Session, users: list[User], projects: list[Project]
) -> list[Task]:
    """Create test tasks."""
    tasks = [
        Task(
...
def create_test_team_members(
    db: Session, users: list[User], projects: list[Project]
) -> list[TeamMember]:
    """Create test team members."""
    team_members = [
        TeamMember(
            user_id=users[0].id, project_id=projects[0].id, role="project_manager"
        ),
        TeamMember(user_id=users[1].id, project_id=projects[0].id, role="developer"),
        TeamMember(user_id=users[0].id, project_id=projects[1].id, role="developer"),
        TeamMember(
            user_id=users[1].id, project_id=projects[1].id, role="project_manager"
        ),
    ]
