...
def init_db() -> None:
    ...
    try:
        ...
        if settings.ENV == "production":
            ...
            if existing_tables:
                logger.info(
                    f"Found {len(existing_tables)} existing tables in production database - skipping table creation"
                )
            else:
...
def get_db_info() -> dict:
    ...
    try:
        ...
        return {"error": str(e), "url": "unknown", "status": "error"}
...
def validate_database_config() -> bool:
    ...
    try:
        ...
        if (
            ...
            == "postgresql://alanmccarthy@localhost:5432/sge_dashboard"
        ):
            logger.error(
                "DATABASE_URL is set to default localhost - this won't work in production"
            )
            return False
