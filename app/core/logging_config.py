...
class StructuredLogger(logging.Logger):
    ...
    def _log_to_json(
        self, level: int, msg: str, args: Any, exc_info: Any = None, extra: Dict = None
    ) -> str:
    ...
    def makeRecord(self, *args, **kwargs):
        ...
        if settings.ENV == "production":
            record.msg = self._log_to_json(
                record.levelno,
                record.msg,
                record.args,
                record.exc_info,
                record.extra if hasattr(record, "extra") else None,
            )
            record.args = ()
        return record
...
def log_database_event(
    ...
    logger.info(
        "Database event", extra={"event_type": event_type, "database_details": details}
    )
