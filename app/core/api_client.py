...
class APIClient:
    ...
    def _update_metrics(self, response: APIResponse, endpoint: str):
        ...
        if response.elapsed:
            total_endpoint_time = endpoint_metrics["avg_response_time"] * (
                endpoint_metrics["requests"] - 1
            )
            endpoint_metrics["avg_response_time"] = (
                total_endpoint_time + response.elapsed
        ...
        **kwargs,
        ...
        while attempt <= self.retry_config.max_retries:
            try:
                ...
                if api_response.success or not self._should_retry(
                    api_response, attempt
                ):
                    return api_response

                logger.warning(
                    f"Request failed (attempt {attempt}): {response.status_code} - {url}"
                )
