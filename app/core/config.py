...
class Settings:
    ...
    JWT_SECRET_KEY: str = (
        os.getenv("JWT_SECRET_KEY", "test-key-for-development-only")
    ...
    RATE_LIMIT_REQUESTS_PER_MINUTE: int = int(
        os.getenv(
            ...
            "100" if os.getenv("ENVIRONMENT", "development") == "development" else "60",
        )
    )
    ...
    INSTAGRAM_BUSINESS_ACCOUNT_ID: Optional[str] = os.getenv(
    ...
    SOCIAL_MEDIA_ENABLED: bool = (
        os.getenv("SOCIAL_MEDIA_ENABLED", "false").lower() == "true"
    )
    SOCIAL_METRICS_CACHE_DURATION: int = int(
        os.getenv("SOCIAL_METRICS_CACHE_DURATION", "3600")
    )  # 1 hour
