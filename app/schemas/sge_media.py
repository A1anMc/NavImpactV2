...
class SgeMediaProjectBase(BaseModel):
    ...
    media_type: str = Field(..., description="Type of media: video, photo, transcript, mixed")
    duration: Optional[str] = Field(None, description="Duration in format '15:30' or '2:30:15'")
    release_date: Optional[date] = Field(None, description="Release date of the media")
    target_audience: Optional[List[str]] = Field(None, description="Target audience list")
...
class SgeDistributionLogBase(BaseModel):
    ...
    publish_date: Optional[date] = Field(None, description="Date when content was published")
    views: Optional[int] = Field(0, description="Number of views")
    reach: Optional[int] = Field(0, description="Number of people reached")
    engagement_rate: Optional[Decimal] = Field(None, description="Engagement rate percentage")
...
class SgePerformanceMetricsBase(BaseModel):
    ...
    watch_time_minutes: Optional[int] = Field(0, description="Total watch time in minutes")
    engagement_rate: Optional[Decimal] = Field(None, description="Engagement rate percentage")
...
class SgeImpactStoryBase(BaseModel):
    ...
    story_type: Optional[str] = Field(None, description="Type of story: testimonial, policy_change, funding_unlocked, community_action")
    ...
    stakeholder_organisation: Optional[str] = Field(None, description="Organisation of the stakeholder")
    impact_date: Optional[date] = Field(None, description="Date when impact occurred")
    quantifiable_outcome: Optional[str] = Field(None, description="Quantifiable outcome of the impact")
...
class SgeClientAccessBase(BaseModel):
    ...
    access_level: Optional[str] = Field("viewer", description="Access level: viewer, analyst, admin")
    allowed_projects: Optional[List[int]] = Field(None, description="List of project IDs the client can access")
    is_active: Optional[bool] = Field(True, description="Whether the client access is active")
