...
class NotionTemplateRequest(BaseModel):
    ...
    parent_page_id: str = Field(..., description="Parent page ID where template will be created")
    project_data: Optional[Dict[str, Any]] = Field(None, description="Project data to populate template")
...
class NotionSyncMappingFilters(BaseModel):
    ...
    media_project_id: Optional[int] = Field(None, description="Filter by media project ID")
    sync_status: Optional[SyncStatus] = Field(None, description="Filter by sync status")
    sync_direction: Optional[SyncDirection] = Field(None, description="Filter by sync direction")
