...
from app.schemas.tag import TagCreate, TagUpdate, TagWithRelations
...
async def get_tags(
    ...
    for tag in query.offset(skip).limit(limit).all():
        ...
        tag_dict.update({"grant_count": grant_count, "project_count": project_count})
...
async def create_tag(
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail=f"Tag with name '{tag_in.name}' already exists"
        )
    ...
    if tag_in.parent_id:
        ...
        if parent.category != tag_in.category:
            raise HTTPException(
                status_code=400, detail="Parent tag must be in the same category"
            )

    # Create tag
    tag = Tag(**tag_in.model_dump(), created_by_id=current_user.id)
    db.add(tag)
    db.commit()
    db.refresh(tag)
...
async def delete_tag(
    ...
    if tag.children:
        raise HTTPException(status_code=400, detail="Cannot delete tag with child tags")
...
async def get_tags_by_category(category: TagCategory, db: Session = Depends(get_db)):
    """
...
async def validate_tag_name(
    name: str, exclude_id: Optional[int] = None, db: Session = Depends(get_db)
):
    """
