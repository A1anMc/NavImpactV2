...
async def get_grants(filters: GrantFilters = Depends(), db: Session = Depends(get_db)):
    ...
    try:
        ...
        if filters.location_eligibility:
            query = query.filter(
                Grant.location_eligibility == filters.location_eligibility
            )
        if filters.status:
...
async def get_grant_match_details(
    grant_id: int, project_profile: ProjectProfile, db: Session = Depends(get_db)
):
...
async def get_grant_dashboard(db: Session = Depends(get_db)):
    ...
    try:
        ...
        categories = GrantsByCategory(
            ...
            by_location={"VIC": 15, "NSW": 10, "Australia": 20},
            by_org_type={
                "social_enterprise": 25,
                "not_for_profit": 15,
                "small_medium_enterprise": 5,
            },
            by_funding_range={"0-10k": 10, "10k-50k": 20, "50k-100k": 10, "100k+": 5},
        )
