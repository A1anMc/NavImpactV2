...
class Grant(Base):
    ...
    def calculate_match_score(self, project_profile: dict) -> dict:
        ...
        if self.location_eligibility and project_profile.get("location"):
            if (
                self.location_eligibility.lower() == project_profile["location"].lower()
                or self.location_eligibility.lower() == "national"
            ):
                score += 20
        ...
        if self.org_type_eligible and project_profile.get("org_type"):
            if (
                project_profile["org_type"]
                in [t.lower() for t in self.org_type_eligible]
                or "any" in self.org_type_eligible
            ):
                score += 20
