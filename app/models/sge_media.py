...
class SgeMediaProject(Base):
    ...
    distribution_logs = relationship(
        "SgeDistributionLog",
        back_populates="media_project",
    ...
    impact_stories = relationship(
        "SgeImpactStory", back_populates="media_project", cascade="all, delete-orphan"
    )
...
class SgeDistributionLog(Base):
    ...
    media_project_id = Column(
        Integer, ForeignKey("sge_media_projects.id", ondelete="CASCADE"), nullable=False
    )
    platform = Column(
        String(100), nullable=False
    )  # 'YouTube', 'Vimeo', 'Festival', 'Social Media'
...
class SgePerformanceMetrics(Base):
    ...
    # Relationships
    media_project = relationship(
        "SgeMediaProject", back_populates="performance_metrics"
    )
...
class SgeImpactStory(Base):
    ...
    )  # 'testimonial', 'policy_change', 'funding_unlocked', 'community_action'
...
class SgeClientAccess(Base):
    ...
    access_level = Column(
        String(50), nullable=True, default="viewer"
    )  # 'viewer', 'analyst', 'admin'
    allowed_projects = Column(
        ARRAY(Integer), nullable=True
    )  # Array of project IDs they can access
