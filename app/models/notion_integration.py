...
class NotionSyncMapping(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    media_project_id = Column(Integer, ForeignKey("sge_media_projects.id", ondelete="CASCADE"), nullable=True, index=True)
    notion_page_id = Column(String(255), nullable=False, index=True)
    notion_database_id = Column(String(255), nullable=True)
    sync_direction = Column(String(20), default="bidirectional", nullable=False)  # 'navimpact_to_notion', 'notion_to_navimpact', 'bidirectional'
    last_sync_at = Column(DateTime(timezone=True), nullable=True)
    sync_status = Column(String(50), default="pending", nullable=False, index=True)  # 'pending', 'syncing', 'completed', 'failed'
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class NotionSyncLog(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    workspace_id = Column(Integer, ForeignKey("notion_workspaces.id", ondelete="CASCADE"), nullable=True, index=True)
    operation_type = Column(String(50), nullable=False)  # 'create_page', 'update_page', 'delete_page', 'sync_project', etc.
    entity_type = Column(String(50), nullable=False)  # 'media_project', 'distribution_log', 'performance_metrics', etc.
