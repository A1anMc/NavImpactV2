...
async def get_media_files(db: Session = Depends(get_db)):
    ...
    return {"message": "Media files endpoint", "files": []}
...
@router.get("/gallery")
async def get_media_gallery(db: Session = Depends(get_db)):
    """Get media gallery."""
    return {"message": "Media gallery endpoint", "gallery": []}


@router.delete("/{media_id}")
async def delete_media(media_id: int, db: Session = Depends(get_db)):
    """Delete media file."""
    return {"message": "Media delete endpoint", "media_id": media_id}
