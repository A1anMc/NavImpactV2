...
from app.models.sge_media import (
    SgeMediaProject, SgeDistributionLog, SgePerformanceMetrics, 
    SgeImpactStory, SgeClientAccess
)
from app.schemas.sge_media import (
    SgeMediaProjectCreate, SgeMediaProjectUpdate, SgeMediaProjectResponse,
    SgeDistributionLogCreate, SgeDistributionLogResponse,
    SgePerformanceMetricsCreate, SgePerformanceMetricsResponse,
    SgeImpactStoryCreate, SgeImpactStoryResponse,
    SgeClientAccessCreate, SgeClientAccessResponse,
    MediaDashboardResponse, MediaImpactReportResponse
)
...
async def create_media_project(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Failed to create media project: {str(e)}")
...
@router.get("/media-projects/{media_project_id}", response_model=SgeMediaProjectResponse)
async def get_media_project(
    ...
    media_project = db.query(SgeMediaProject).filter(SgeMediaProject.id == media_project_id).first()
    if not media_project:
        raise HTTPException(status_code=404, detail="Media project not found")
    return media_project


@router.put("/media-projects/{media_project_id}", response_model=SgeMediaProjectResponse)
async def update_media_project(
    ...
    db_media_project = db.query(SgeMediaProject).filter(SgeMediaProject.id == media_project_id).first()
...
async def add_distribution_log(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Failed to create distribution log: {str(e)}")
...
async def add_performance_metrics(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Failed to create performance metrics: {str(e)}")
...
async def add_impact_story(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Failed to create impact story: {str(e)}")
...
async def create_client_access(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Failed to create client access: {str(e)}")
...
async def get_media_dashboard(
    ...
    try:
        ...
        total_views = db.query(SgePerformanceMetrics.views).filter(
            SgePerformanceMetrics.views.isnot(None)
        ).scalar() or 0
        active_distributions = db.query(SgeDistributionLog).count()
        impact_stories = db.query(SgeImpactStory).count()
        
        # Get recent projects
        recent_projects = db.query(SgeMediaProject).order_by(
            SgeMediaProject.created_at.desc()
        ).limit(5).all()
        
        # Get recent stories
        recent_stories = db.query(SgeImpactStory).order_by(
            SgeImpactStory.created_at.desc()
        ).limit(5).all()
        
        # Placeholder for chart data (would be implemented with actual aggregation queries)
        distribution_data = {"platforms": ["YouTube", "Vimeo", "Social"], "counts": [10, 5, 8]}
...
async def get_media_impact_report(
    ...
    try:
        ...
        media_projects = db.query(SgeMediaProject).filter(
            SgeMediaProject.project_id == project_id
        ).all()
        ...
        for mp in media_projects:
            metrics = db.query(SgePerformanceMetrics).filter(
                SgePerformanceMetrics.media_project_id == mp.id
            ).all()
        ...
        impact_stories = []
        for mp in media_projects:
            stories = db.query(SgeImpactStory).filter(
                SgeImpactStory.media_project_id == mp.id
            ).all()
            impact_stories.extend(stories)
        
        # Placeholder data for summaries
        distribution_summary = {"platforms": 3, "total_distributions": 15}
        performance_summary = {"avg_views": total_views // len(media_projects) if media_projects else 0}
