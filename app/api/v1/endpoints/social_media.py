...
async def get_instagram_metrics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch Instagram metrics")
...
async def get_instagram_followers(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to fetch Instagram follower count"
        )
...
async def get_instagram_account_info(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to fetch Instagram account information"
        )
...
async def get_social_media_status(
    ...
    status = {
        "instagram": {
            ...
            "status": (
                "configured" if instagram_service.access_token else "not_configured"
            ),
        },
        "overall_status": (
            "partial" if instagram_service.access_token else "not_configured"
        ),
    }
