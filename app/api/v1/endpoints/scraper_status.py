...
@router.post("/run")
def run_scrapers(background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
...
def get_scraper_sources(db: Session = Depends(get_db)):
    ...
    latest_logs = (
        db.query(
            ...
            func.sum(case((ScraperLog.status == "error", 1), else_=0)).label(
                "error_count"
            ),
        )
        .group_by(ScraperLog.source_name)
    ...
    return [
        {
            ...
            "avg_duration_seconds": (
                round(float(log.avg_duration), 2) if log.avg_duration else None
            ),
            ...
            "error_rate": (
                round((log.error_count / log.total_runs) * 100, 2)
                if log.total_runs > 0
                else 0
            ),
        }
...
def get_source_history(
    source_name: str, limit: int = 10, db: Session = Depends(get_db)
):
    ...
    if not logs:
        raise HTTPException(
            status_code=404, detail=f"No logs found for source: {source_name}"
        )
