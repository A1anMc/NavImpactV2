...
def get_sustainability_metrics(
    ...
    reporting_period: Optional[str] = Query(
        None, description="Filter by reporting period"
    ),
...
def create_sustainability_metric(
    ...
    db_metric = SustainabilityMetric(**metric.dict(), organisation_id=current_user.id)
    db.add(db_metric)
    db.commit()
...
def create_sustainability_policy(
    ...
    db_policy = SustainabilityPolicy(**policy.dict(), organisation_id=current_user.id)
    db.add(db_policy)
    db.commit()
...
def get_action_plans(
    ...
    query = db.query(ActionPlan).filter(ActionPlan.organisation_id == current_user.id)
...
def create_action_plan(
    ...
    db_plan = ActionPlan(**plan.dict(), organisation_id=current_user.id)
    db.add(db_plan)
    db.commit()
    db.refresh(db_plan)
    return db_plan
...
def update_action_plan(
    ...
    db_plan = (
        db.query(ActionPlan)
        .filter(ActionPlan.id == plan_id, ActionPlan.organisation_id == current_user.id)
        .first()
    )
...
def create_performance_target(
    ...
    db_target = PerformanceTarget(**target.dict(), organisation_id=current_user.id)
    db.add(db_target)
    db.commit()
...
def create_assurance_log(
    ...
    db_assurance = AssuranceLog(**assurance.dict(), organisation_id=current_user.id)
    db.add(db_assurance)
    db.commit()
...
def get_sustainability_overview(
    ...
    total_metrics = (
        db.query(SustainabilityMetric)
        .filter(SustainabilityMetric.organisation_id == current_user.id)
        .count()
    )
    ...
    total_policies = (
        db.query(SustainabilityPolicy)
        .filter(SustainabilityPolicy.organisation_id == current_user.id)
        .count()
    )
    ...
    total_action_plans = (
        db.query(ActionPlan)
        .filter(ActionPlan.organisation_id == current_user.id)
        .count()
    )
    ...
    total_targets = (
        db.query(PerformanceTarget)
        .filter(PerformanceTarget.organisation_id == current_user.id)
        .count()
    )
    ...
    recent_assurance = (
        ...
        .filter(AssuranceLog.organisation_id == current_user.id)
        .order_by(AssuranceLog.assurance_date.desc())
        .first()
    )
...
def get_esrs_dashboard(
    ...
    return ESRSDashboard(environment=environment, social=social, governance=governance)
...
def export_sustainability_data(
    ...
    action_plans = (
        db.query(ActionPlan).filter(ActionPlan.organisation_id == current_user.id).all()
    )
