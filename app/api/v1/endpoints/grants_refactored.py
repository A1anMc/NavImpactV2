...
from app.schemas.grant import GrantResponse, GrantListResponse, GrantRecommendationResponse
...
def get_grant_service(
    ...
    class MockNotificationService(NotificationServiceInterface):
        ...
        def send_grant_notification(self, user_id: int, grant_id: int, notification_type: str) -> bool:
            return True
        
        def send_bulk_notification(self, user_ids: list[int], message: str, notification_type: str = "info") -> dict[int, bool]:
...
def get_grants(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving grants: {str(e)}")
...
@router.get("/grants/recommendations/", response_model=List[GrantRecommendationResponse])
...
def get_upcoming_deadlines(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting upcoming deadlines: {str(e)}") 