...
class ScreenAustraliaGrantScraper(BaseGrantScraper):
    ...
    def _parse_grants(self, content: str) -> List[Grant]:
        ...
        grant_containers = soup.find_all(
            ...
            and any(
                keyword in x.lower()
                for keyword in ["grant", "funding", "opportunity", "program"]
            ),
        )
        ...
        if not grant_containers:
            ...
            grant_containers = soup.find_all(
                ...
                and any(
                    keyword in x.lower()
                    for keyword in ["funding", "grant", "support", "opportunity"]
                ),
            )
    ...
    def _extract_grant_from_container(self, container) -> Optional[Grant]:
        ...
        try:
            ...
            description_elem = container.find(
                ["p", "div"], class_=lambda x: x and "description" in x.lower()
            )
            if not description_elem:
                description_elem = container.find("p")

            description = (
                description_elem.get_text().strip() if description_elem else None
            )
    ...
    def _extract_application_url(self, container) -> Optional[str]:
        ...
        app_links = container.find_all(
            "a",
            href=True,
            string=lambda x: x
            and any(
                keyword in x.lower()
                for keyword in ["apply", "application", "submit", "register"]
            ),
        )
        ...
        for link in all_links:
            ...
            if any(
                ...
                for keyword in ["apply", "application", "submit"]
            ):
                return href
    ...
    def _determine_org_types(self, container) -> List[str]:
        ...
        if any(
            keyword in text
            for keyword in ["production company", "producer", "filmmaker"]
        ):
