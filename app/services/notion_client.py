...
class NotionAPIClient:
    ...
    def search(
        self, query: str = "", filter_type: str = "page", page_size: int = 100
    ) -> Dict[str, Any]:
...
class NotionTemplateManager:
    ...
    def create_media_project_template(
        self, parent_page_id: str, project_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        properties = {
            ...
            "Status": {"select": {"name": project_data.get("status", "Planning")}},
            "Project Type": {
                "select": {"name": project_data.get("project_type", "Video")}
            },
            "Start Date": {"date": {"start": project_data.get("start_date")}},
            "End Date": {"date": {"start": project_data.get("end_date")}},
            "Budget": {"number": project_data.get("budget", 0)},
            "Target Audience": {"number": project_data.get("target_audience", 0)},
            "Team Members": {
                "multi_select": [
            ...
            "Distribution Channels": {
                "multi_select": [
                    {"name": channel}
                    for channel in project_data.get("distribution_channels", [])
                ]
            },
        }
        ...
        content = [
            ...
            },
            {
                ...
                "heading_2": {
                    "rich_text": [
                        {"type": "text", "text": {"content": "Distribution Log"}}
                    ]
                },
            },
            {"object": "block", "type": "table_of_contents", "table_of_contents": {}},
            {
                ...
                "heading_2": {
                    "rich_text": [
                        {"type": "text", "text": {"content": "Performance Metrics"}}
                    ]
                },
            },
            {
    ...
    def create_distribution_log_template(
        self, parent_page_id: str, log_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        properties = {
            "Activity": {
                "title": [
                    {
                        "text": {
                            "content": log_data.get(
                                "activity_type", "Distribution Activity"
                            )
                        }
                    }
                ]
            },
            "Channel": {"select": {"name": log_data.get("channel", "Social Media")}},
            "Date": {"date": {"start": log_data.get("date")}},
            "Reach": {"number": log_data.get("reach", 0)},
            "Engagement": {"number": log_data.get("engagement", 0)},
            "Conversions": {"number": log_data.get("conversions", 0)},
        }
                    ...
                    "rich_text": [
                        {
                            ...
                            "text": {
                                "content": log_data.get(
                                    "notes", "Distribution activity notes."
                                )
                            },
                        }
                    ]
...
class NotionSyncManager:
    ...
    def __init__(
        self, client: NotionAPIClient, template_manager: NotionTemplateManager
    ):
        self.client = client
        self.template_manager = template_manager

    def sync_media_project(
        self, project_data: Dict[str, Any], notion_page_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully synced project {project_data.get('id')} to Notion page {notion_page_id}"
            )
            return result

        except NotionAPIError as e:
            logger.error(
                f"Failed to sync project {project_data.get('id')} to Notion: {e.message}"
            )
            raise

    def _map_project_to_notion_properties(
        self, project_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Map NavImpact project data to Notion properties."""
        return {
            "Title": {"title": [{"text": {"content": project_data.get("title", "")}}]},
            "Status": {"select": {"name": project_data.get("status", "Active")}},
            "Project Type": {
                "select": {"name": project_data.get("project_type", "Video")}
            },
            "Start Date": {"date": {"start": project_data.get("start_date")}},
            ...
            "Budget": {"number": project_data.get("budget", 0)},
            "Target Audience": {"number": project_data.get("target_audience", 0)},
            "Impact Score": {"number": project_data.get("impact_score", 0)},
        }

    def sync_distribution_log(
        self, log_data: Dict[str, Any], notion_page_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully synced distribution log {log_data.get('id')} to Notion"
            )
            return result

        except NotionAPIError as e:
            logger.error(
                f"Failed to sync distribution log {log_data.get('id')} to Notion: {e.message}"
            )
            raise

    def _map_distribution_to_notion_properties(
        self, log_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Map distribution log data to Notion properties."""
        return {
            "Activity": {
                "title": [{"text": {"content": log_data.get("activity_type", "")}}]
            },
            "Channel": {"select": {"name": log_data.get("channel", "")}},
            "Date": {"date": {"start": log_data.get("date")}},
            "Reach": {"number": log_data.get("reach", 0)},
            "Engagement": {"number": log_data.get("engagement", 0)},
            "Conversions": {"number": log_data.get("conversions", 0)},
