...
class BusinessGovScraper(BaseScraper):
    ...
    async def scrape(self) -> List[Grant]:
        ...
        try:
            ...
            logger.info(
                f"Successfully scraped {len(saved_grants)} grants from Business.gov.au"
            )
            return saved_grants
    ...
    async def _extract_grant_from_element(
        self, element: BeautifulSoup, url: str
    ...
    async def _extract_main_page_grant(
        self, soup: BeautifulSoup, url: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _extract_funding_purpose(self, text: str) -> List[str]:
        ...
        if any(
            word in text_lower
            for word in ["export", "international", "market development"]
        ):
