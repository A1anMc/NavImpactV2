...
class MediaInvestmentScraper(BaseScraper):
        ...
        try:
            ...
            for opportunity in unique_opportunities:
                if self._validate_grant_data(opportunity):
                    ...
                    logger.warning(
                        f"Invalid opportunity data: {opportunity.get('title', 'Unknown')}"
                    )
            ...
            logger.info(
                f"Successfully scraped {len(saved_opportunities)} media investment opportunities"
            )
            ...
            try:
                ...
                logger.info(
                    f"Fallback: saved {len(saved_opportunities)} known opportunities"
                )
                return saved_opportunities
    ...
    async def _scrape_all_media_companies(self) -> List[Dict[str, Any]]:
        ...
        tasks = []
        for i, (company_name, company_config) in enumerate(
            self.media_companies.items()
        ):
            delay = i * 5  # 5 seconds between each company
            task = asyncio.create_task(
                self._scrape_company_with_delay(company_name, company_config, delay)
            )
            tasks.append(task)
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                ...
                logger.info(
                    f"Successfully scraped {len(result)} opportunities from {company_name}"
                )
                all_opportunities.extend(result)

        return all_opportunities

    async def _scrape_company_with_delay(
        self, company_name: str, company_config: Dict, delay: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _scrape_company(
        self, company_name: str, company_config: Dict
    ) -> List[Dict[str, Any]]:
        ...
        for endpoint in company_config["endpoints"]:
            try:
                ...
                if endpoint_opportunities:
                    ...
                    logger.info(
                        f"Found {len(endpoint_opportunities)} opportunities from {url}"
                    )
                ...
                logger.error(
                    f"Error scraping {company_name} endpoint {endpoint}: {str(e)}"
                )
                continue
    ...
    async def _parse_abc(self, soup: BeautifulSoup, url: str) -> List[Dict[str, Any]]:
        ...
        opportunity_containers = soup.find_all(
            ["div", "section"],
            class_=re.compile(r"opportunity|funding|innovation|development", re.I),
        )

        for container in opportunity_containers:
            try:
                opportunity_data = await self._extract_opportunity_from_container(
                    container, url
                )
                if opportunity_data:
    ...
    async def _parse_sbs(self, soup: BeautifulSoup, url: str) -> List[Dict[str, Any]]:
        ...
        opportunity_containers = soup.find_all(
            ["div", "article"],
            class_=re.compile(r"opportunity|funding|talent|development", re.I),
        )
    ...
    async def _parse_nine(self, soup: BeautifulSoup, url: str) -> List[Dict[str, Any]]:
        ...
        opportunity_containers = soup.find_all(
            ["div", "section"],
            class_=re.compile(r"opportunity|development|innovation", re.I),
        )
    ...
    async def _parse_seven(self, soup: BeautifulSoup, url: str) -> List[Dict[str, Any]]:
        ...
        opportunity_containers = soup.find_all(
            ["div", "section"],
            class_=re.compile(r"opportunity|development|talent", re.I),
        )
    ...
    async def _parse_ten(self, soup: BeautifulSoup, url: str) -> List[Dict[str, Any]]:
        ...
        opportunity_containers = soup.find_all(
            ["div", "section"],
            class_=re.compile(r"opportunity|development|creator", re.I),
        )
    ...
    async def _parse_foxtel(
        ...
        opportunity_containers = soup.find_all(
            ["div", "section"],
            class_=re.compile(r"opportunity|development|production", re.I),
        )
    ...
    async def _parse_news_corp(
        ...
        opportunity_containers = soup.find_all(
            ["div", "section"],
            class_=re.compile(r"opportunity|innovation|digital", re.I),
        )
    ...
    async def _parse_sca(self, soup: BeautifulSoup, url: str) -> List[Dict[str, Any]]:
        ...
        # Look for opportunity information
        opportunity_containers = soup.find_all(
            ["div", "section"], class_=re.compile(r"opportunity|content|audio", re.I)
        )
    ...
    async def _parse_stan(self, soup: BeautifulSoup, url: str) -> List[Dict[str, Any]]:
        ...
        opportunity_containers = soup.find_all(
            ["div", "section"],
            class_=re.compile(r"opportunity|original|streaming", re.I),
        )
    ...
    async def _parse_generic_media(
        self, soup: BeautifulSoup, url: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _extract_opportunity_from_container(
        ...
        try:
            ...
            # Extract description
            description_elem = container.find(
                ["p", "div"], class_=re.compile(r"desc|summary|content", re.I)
            )
    ...
    async def _process_known_opportunities(self) -> List[Dict[str, Any]]:
        ...
        for opportunity_data in self.known_opportunities:
            try:
                ...
                logger.error(
                    f"Error processing known opportunity {opportunity_data.get('title', 'Unknown')}: {str(e)}"
                )
                continue
