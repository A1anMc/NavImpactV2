...
class PhilanthropicScraper(BaseScraper):
    ...
    async def scrape(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Successfully scraped {len(saved_grants)} philanthropic grants"
            )
            return saved_grants
    ...
    async def _scrape_all_foundations(self) -> List[Dict[str, Any]]:
        ...
        tasks = []
        for i, (foundation_name, foundation_config) in enumerate(
            self.foundations.items()
        ):
            delay = i * 3  # 3 seconds between each foundation
            task = asyncio.create_task(
                self._scrape_foundation_with_delay(
                    foundation_name, foundation_config, delay
                )
            )
            tasks.append(task)
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                ...
                logger.info(
                    f"Successfully scraped {len(result)} grants from {foundation_name}"
                )
                all_grants.extend(result)

        return all_grants

    async def _scrape_foundation_with_delay(
        self, foundation_name: str, foundation_config: Dict, delay: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _scrape_foundation(
        self, foundation_name: str, foundation_config: Dict
    ) -> List[Dict[str, Any]]:
        ...
        for endpoint in foundation_config["endpoints"]:
            try:
                ...
                logger.error(
                    f"Error scraping {foundation_name} endpoint {endpoint}: {str(e)}"
                )
                continue
    ...
    async def _scrape_endpoint(
        self, foundation_name: str, url: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _parse_lmcf(self, soup: BeautifulSoup, url: str) -> List[Dict[str, Any]]:
        ...
        grant_containers = soup.find_all(
            ["div", "section"], class_=re.compile(r"grant|funding|program", re.I)
        )
    ...
    async def _parse_myer(self, soup: BeautifulSoup, url: str) -> List[Dict[str, Any]]:
        ...
        grant_containers = soup.find_all(
            ["div", "article"], class_=re.compile(r"grant|program|funding", re.I)
        )
    ...
    async def _parse_hms(self, soup: BeautifulSoup, url: str) -> List[Dict[str, Any]]:
        ...
        # Look for grant information
        grant_containers = soup.find_all(
            ["div", "section"], class_=re.compile(r"grant|funding", re.I)
        )
    ...
    async def _parse_australia_council(
        ...
        # Look for grant information
        grant_containers = soup.find_all(
            ["div", "article"], class_=re.compile(r"grant|funding|opportunity", re.I)
        )
    ...
    async def _parse_generic_foundation(
        self, soup: BeautifulSoup, url: str
    ) -> List[Dict[str, Any]]:
        ...
        for selector in selectors:
            ...
            for container in containers:
                try:
                    grant_data = await self._extract_grant_from_container(
                        container, url
                    )
                    if grant_data:
                        grants.append(grant_data)
    ...
    async def _extract_grant_from_container(
    ...
    def _determine_industry_focus(self, text: str) -> str:
        ...
        if any(
            word in text_lower
            for word in ["art", "creative", "cultural", "music", "film", "theatre"]
        ):
            return "arts"
        elif any(
            word in text_lower
            for word in ["education", "school", "student", "learning"]
        ):
            return "education"
        elif any(
            word in text_lower for word in ["health", "medical", "wellbeing", "mental"]
        ):
            return "health"
        elif any(
            word in text_lower for word in ["environment", "sustainability", "climate"]
        ):
            return "environment"
        elif any(
            word in text_lower for word in ["technology", "innovation", "digital"]
        ):
            return "technology"
        else:
            return "community"
    ...
    async def _process_known_grants(self) -> List[Dict[str, Any]]:
        ...
        for grant_data in self.known_grants:
            try:
                ...
                logger.error(
                    f"Error processing known grant {grant_data.get('title', 'Unknown')}: {str(e)}"
                )
                continue
