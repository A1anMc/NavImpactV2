...
class CouncilScraper(BaseScraper):
    ...
    async def _scrape_all_councils(self) -> List[Dict[str, Any]]:
        ...
        for i, (council_name, council_config) in enumerate(self.councils.items()):
            ...
            task = asyncio.create_task(
                self._scrape_council_with_delay(council_name, council_config, delay)
            )
            tasks.append(task)
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                ...
                logger.info(
                    f"Successfully scraped {len(result)} grants from {council_name}"
                )
                all_grants.extend(result)

        return all_grants

    async def _scrape_council_with_delay(
        self, council_name: str, council_config: Dict, delay: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _scrape_council(
        self, council_name: str, council_config: Dict
    ) -> List[Dict[str, Any]]:
        ...
        for endpoint in council_config["endpoints"]:
            try:
                ...
                logger.error(
                    f"Error scraping {council_name} endpoint {endpoint}: {str(e)}"
                )
                continue
    ...
    async def _parse_generic_council(
        self, soup: BeautifulSoup, url: str
    ) -> List[Dict[str, Any]]:
    ...
    def _determine_industry_focus(self, text: str) -> str:
        ...
        if any(
            ...
            word in text_lower
            for word in ["business", "entrepreneur", "startup", "commerce"]
        ):
            return "business"
        elif any(
            word in text_lower
            for word in ["environment", "sustainability", "climate", "green"]
        ):
            return "environment"
        elif any(
            word in text_lower for word in ["sport", "recreation", "fitness", "health"]
        ):
            return "sport"
        elif any(
