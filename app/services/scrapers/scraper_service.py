    ...
    async def scrape_source(self, source_name: str) -> Dict:
        ...
        try:
            ...
            log.complete(
                ...
                metadata={
                    "urls_scraped": (
                        scraper.urls_scraped
                        if hasattr(scraper, "urls_scraped")
                        else None
                    ),
                    "rate_limits": (
                        scraper.rate_limits if hasattr(scraper, "rate_limits") else None
                    ),
                },
            )
            ...
            log.complete(status="error", error_message=str(e))
            raise
