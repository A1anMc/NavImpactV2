                    ...
                    try:
                        ...
                        async with session.get(url) as response:
                            ...
                            if "arts.gov.au" in url:
                                ...
                                grants.extend(
                                    await self._parse_screen_australia(soup, url)
                                )
        ...
        try:
            ...
            for element in grant_elements:
                try:
                    ...
                    logger.error(
                        f"Error parsing grant {title if 'title' in locals() else 'Unknown'}: {str(e)}"
                    )
                    continue
        ...
        try:
            async with aiohttp.ClientSession(headers=self.headers) as session:
                async with session.get(url) as response:
                    if response.status != 200:
                        logger.error(
                            f"Failed to fetch grant details: {response.status}"
                        )
                        return None
                    ...
                    if contact:
                        ...
                        if email:
                            details["contact_email"] = email["href"].replace(
                                "mailto:", ""
                            )
