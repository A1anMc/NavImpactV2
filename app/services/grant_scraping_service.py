    ...
    def scrape_all_sources(self) -> ScrapingSummary:
        ...
        for scraper in self.scrapers:
            ...
            if result.success:
                ...
                self.logger.info(f"Successfully scraped {len(result.grants)} grants from {result.source_name}")
            else:
                failed_sources += 1
                self.logger.error(f"Failed to scrape {result.source_name}: {result.error_message}")
        
        total_duration = (datetime.utcnow() - start_time).total_seconds()
        
        self.logger.info(f"Scraping completed: {successful_sources} successful, {failed_sources} failed, {total_grants} total grants")
        ...
        return [
            {
                ...
                'display_name': getattr(scraper, 'source_display_name', scraper.source_name),
