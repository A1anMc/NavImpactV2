#!/usr/bin/env python3
"""
Script to seed the industry_news database with sample data
"""
...
def seed_news_database():
    ...
    print("🗞️  Starting news database seeding...")
    ...
    try:
        ...
        if existing_count > 0:
            response = input("⚠️  News items already exist. Clear and reseed? (y/N): ")
            if response.lower() != 'y':
            ...
            print("🗑️  Cleared existing news items.")
        
        # Get sample data
        news_data = get_sample_news_data()
        print(f"📰 Preparing to seed {len(news_data)} news items...")
        ...
        # Commit changes
        db.commit()
        print(f"\n🎉 Successfully seeded {created_count} news items!")
        
        # Show summary by sector
        print("\n📈 News items by sector:")
        ...
        print(f"\n✅ News database seeding complete!")
